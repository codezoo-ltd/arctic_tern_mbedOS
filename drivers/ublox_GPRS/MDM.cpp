...
{
#ifdef MDM_DEBUG
    if ( (mbed_trace_config_get() & TRACE_LEVEL_DEBUG) ) 
    {
	char str[32];
	memset(str, 0, sizeof(str));
	snprintf(str, len-1, "%s", buf);
	//memcpy(str, buf, len);
        TRACE("\t%s %10.3f AT send    ", str, _debugTime.read_ms()*0.001);
    }
#endif
...
#ifdef MDM_DEBUG
        if ((mbed_trace_config_get() & TRACE_LEVEL_DEBUG) && (ret != WAIT) && (ret != NOT_FOUND))
        {
            int len = LENGTH(ret);
            int type = TYPE(ret);
            const char* s = (type == TYPE_UNKNOWN)? "UNK": 
                            ...
                            (type == TYPE_PLUS)   ? " + ": 
                            (type == TYPE_PROMPT) ? " > ": "..." ;
            TRACE("%10.3f AT read %s", _debugTime.read_ms()*0.001, s);
        }
#endif        
...
bool MDMParser::connect(
            ...
            PinName pn, PinName r_pn)
{
    bool ok = init(simpin, NULL, pn, r_pn);  
#ifdef MDM_DEBUG
    if (mbed_trace_config_get()&TRACE_LEVEL_DEBUG) dumpDevStatus(&_dev);
#endif
    if (!ok)
        return false;
    ok = registerNet();
#ifdef MDM_DEBUG
    if (mbed_trace_config_get()&TRACE_LEVEL_DEBUG) dumpNetStatus(&_net);
#endif
    if (!ok)
        return false;
    IP ip = join(apn,username,password,auth);
#ifdef MDM_DEBUG
    if (mbed_trace_config_get()&TRACE_LEVEL_DEBUG) dumpIp(ip);
#endif
    if (ip == NOIP)
        return false; 
    return true;
...
bool MDMParser::init(const char* simpin, DevStatus* status, PinName pn, PinName r_pn)
{
    int i = 10;
    LOCK();
    memset(&_dev, 0, sizeof(_dev));
